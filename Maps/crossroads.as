//crossroads
//============
//two intersecting lines of single stars
//No Dust
...
void makeMap(Galaxy@ Glx) {
	...
	for(; sysIndex < sysCount; ++sysIndex) {
		...
		else if(sysIndex > (sysCount / 2) && sysIndex < uint(sysCount / 1.2)) {
			xxx = (getGameSetting("MAP_SYSTEM_SPACING", 3000.f) + rad) * side;
			if(xxx < 0)
				xxx = xxx * -2;
		}		
		...
		if(sysIndex < (sysCount / 2))
			...
			yyy = -1 * ((1000.f+sysIndex)^2.f) + (getGameSetting("MAP_SYSTEM_SPACING", 3000.f));
