...
const string@ strMetals = "Metals", strElects = "Electronics", strAdvParts = "AdvParts", strAmmo = "Ammo", strFuel = "Fuel";
const string@ strLabor = "Labr";
...
void tick(float time) {
	...
	if (curObj.getConstructionQueueSize() > 0) {
		...
		curObj.getConstructionCost(0, strMetals, done, req);
		activeCost.update(1, done, req);

		curObj.getConstructionCost(0, strElects, done, req);
		activeCost.update(2, done, req);

		curObj.getConstructionCost(0, strAdvParts, done, req);
		activeCost.update(0, done, req);
		
		curObj.getConstructionCost(0, strLabor, done, req);
