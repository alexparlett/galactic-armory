...
const string@ strFood = "Food", strOre = "Ore", strDamage = "Damage", strH3 = "H3";
...
const string@ strFuel = "Fuel", strAmmo = "Ammo", strFuelG = "FuelG", strAmmoG = "AmmoG", strH3FuelGen = "H3FuelG";
string@ strUnique = "Unique";

const string@ strDisableCivilActs = "disable_civil_acts", strFastConsumption = "fast_consumption", strSlowConsumption = "low_consume", strNoFood = "no_food";
...
string@[] resNames = { "#advparts", "#electronics", "#metals", "#food", "#goods", "#luxuries", "#fuel", "#ammo"};
uint[] resColors = { 0xffc2e4ff, 0xfff6ff00, 0xffeaeaea, 0xff197b30, 0xff987433, 0xffffb9b9, 0xffff8500, 0xffb54f4a};
/* }}} */
...
class PlanetWindow : ScriptedGuiHandler {
	...
	// Improvements tab
	GuiPanel@ improvPanel;
	
	GuiButton@ terraTab;
	
	GuiListBox@ terraList;
	
	GuiImage@ lockIcon;
	GuiImage@ unlockIcon;	
	
	GuiExtText@ improvName;
	GuiExtText@ improvDesc;
	GuiExtText@ improvBuild;
	GuiStaticText@ impCost;
	ResourceGrid@ improvCost;
	GuiButton@ improvPause;
	GuiButton@ improvCancel;
	...
	void init(GuiElement@ ele) {
		...
		@structuresText = GuiStaticText(recti(0, 0, 320, 20), null, false, false, false, topPanel);
		...
		@improvTab = TabButton(recti(), localize("#PL_IMPROVEMENTS"), topPanel);
		...
		@improvPanel = GuiPanel(recti(0, 20, 200, 70), false, SBM_Invisible, SBM_Invisible, ele);
		improvPanel.fitChildren();
		improvPanel.setVisible(false);
		
		@terraList = GuiListBox(recti(0, 24, 0, 0), true, improvPanel);
		
		@lockIcon = GuiImage(pos2di(274, 35), "lock_gui", improvPanel);
		lockIcon.setVisible(false);
		@unlockIcon = GuiImage(pos2di(274, 35), "unlock_gui", improvPanel);
		unlockIcon.setVisible(false);		

		@improvBuild = GuiExtText(recti(pos2di(280, 5), dim2di(234, 28)), improvPanel);
		@improvName = GuiExtText(recti(pos2di(340, 34), dim2di(390, 61)), improvPanel);	
		@improvDesc = GuiExtText(recti(pos2di(285, 102), dim2di(270, 121)), improvPanel);
		@impCost = GuiStaticText(recti(pos2di(500,72), dim2di(70, 121)), localize("#PL_IMPROV_COST"), false, false, false, improvPanel);
		@improvCost = ResourceGrid(improvPanel, pos2di(), 1);
		improvCost.setCellSize(dim2di(112, 17));
		improvCost.add(SR_Metals, 0);
		improvCost.add(SR_Electronics, 0);
		improvCost.add(SR_AdvParts, 0);		
		improvCost.setVisible(false);
		
		@improvPause = Button(dim2di(134, 22), localize("#PL_IMPROV_PAUSE"), improvPanel);
		@improvCancel = Button(dim2di(134, 22), localize("#PL_IMPROV_CANCEL"), improvPanel);

		@terraTab = TabButton(recti(1, 1, 270, 19), localize("#PL_TerraList"), improvPanel);
		terraTab.setPressed(true);		
		...
		// Variable headers
		@txt = GuiStaticText(recti(pos2di(12, 12), dim2di(140, 18)), localize("#PL_PopMult"), false, false, false, econPanel);
		@econ_popmult = GuiStaticText(recti(pos2di(170, 12), dim2di(140, 18)), null, false, false, false, econPanel);
		txt.setToolTip(localize("#PLTT_PopMult"));
		txt.orphan(true);

		@txt = GuiStaticText(recti(pos2di(12, 32), dim2di(140, 18)), localize("#PL_MoodMult"), false, false, false, econPanel);
		@econ_moodmult = GuiStaticText(recti(pos2di(170, 32), dim2di(140, 18)), null, false, false, false, econPanel);
		txt.setToolTip(localize("#PLTT_MoodMult"));
		txt.orphan(true);

		@txt = GuiStaticText(recti(pos2di(12, 52), dim2di(140, 18)), localize("#PL_WorkersMult"), false, false, false, econPanel);
		@econ_workersmult = GuiStaticText(recti(pos2di(170, 52), dim2di(140, 18)), null, false, false, false, econPanel);
		txt.setToolTip(localize("#PLTT_WorkersMult"));
		txt.orphan(true);

		@txt = GuiStaticText(recti(pos2di(365, 12), dim2di(140, 18)), localize("#PL_Population"), false, false, false, econPanel);
		@econ_pop = GuiStaticText(recti(pos2di(523, 12), dim2di(140, 18)), null, false, false, false, econPanel);
		txt.setToolTip(localize("#PLTT_Population"));
		txt.orphan(true);

		@txt = GuiStaticText(recti(pos2di(365, 32), dim2di(140, 18)), localize("#PL_TradeUsed"), false, false, false, econPanel);
		@econ_trade = GuiStaticText(recti(pos2di(523, 32), dim2di(140, 18)), null, false, false, false, econPanel);
		txt.setToolTip(localize("#PLTT_TradeUsed"));
		txt.orphan(true);

		@allowImport = GuiCheckBox(true, recti(pos2di(365, 53), dim2di(135, 18)), localize("#PL_AllowImport"), econPanel);
		allowImport.setToolTip(localize("#PLTT_AllowImport"));

		@allowExport = GuiCheckBox(true, recti(pos2di(523, 53), dim2di(135, 18)), localize("#PL_AllowExport"), econPanel);
	...
	void syncPosition(dim2di size) {
		...
		name.setSize(dim2di(topWidth - 120 - 440, 20));
		blockadedText.setPosition(pos2di((topWidth - 438) / 2 , 2));
		structuresText.setPosition(pos2di(topWidth - 538, 2));
		...
		improvTab.setPosition(pos2di(8 + tabSize*2, 127));		
		econTab.setPosition(pos2di(12 + tabSize*3, 127));
		
		queueTab.setSize(dim2di(tabSize, 18));
		structsTab.setSize(dim2di(tabSize, 18));
		improvTab.setSize(dim2di(tabSize, 18));
		...
		hovered_cost.setPosition(pos2di(10, queueEle.getSize().height - 26));
		hovered_cost.setCellSize(dim2di(queueEle.getSize().width / 6, 17));
		...
		activeCost.setPosition(pos2di(size.width - 232, 6));
		...
		// Improvments tab
		improvPanel.setPosition(topLeft);
		improvPanel.setSize(size);
		
		terraList.setSize(dim2di(269, size.height - 50));
		
		improvBuild.setSize(dim2di(234, 28));
		improvName.setSize(dim2di(size.width - 290, 60));
		improvDesc.setSize(dim2di(size.width - 375, 120));
		impCost.setPosition(pos2di(size.width - 53, 98));
		improvCost.setPosition(pos2di(size.width - 68, 126));
		
		improvPause.setPosition(pos2di(1, size.height - 22));
		improvCancel.setPosition(pos2di(135, size.height - 22));		

		lockIcon.setSize(dim2di(55,55));
		unlockIcon.setSize(dim2di(55,55));
	...
	void draw(GuiElement@ ele) {
		...
		else if (improvPanel.isVisible()) {
			//Left
			drawVSep(recti(topLeft + pos2di(274, 168), dim2di(7, size.height - 174)));
			drawHSep(recti(pos2di(topLeft.x + 6, topLeft.y + 186), dim2di(270, 7)));
			drawHSep(recti(pos2di(topLeft.x + 6, botRight.y - 35), dim2di(270, 7)));
			
			//Right
			drawLightArea(recti(topLeft + pos2di(279, 200), pos2di(topLeft.x + 338, topLeft.y + 258)));
			drawLightArea(recti(topLeft + pos2di(341, 200), pos2di(botRight.x - 7, topLeft.y + 258)));	
			drawLightArea(recti(topLeft + pos2di(279, 261), pos2di(botRight.x - 89, topLeft.y + 379)));	
			drawLightArea(recti(topLeft + pos2di(279, 170), pos2di(topLeft.x + 518, topLeft.y + 198)));
			drawDarkArea(recti(topLeft + pos2di(279, 383), botRight - pos2di(7,7)));	
			drawDarkArea(recti(topLeft + pos2di(518, 168), dim2di(size.width - 524, 30)));
			drawDarkArea(recti(topLeft + pos2di(size.width - 87, 259), pos2di(botRight.x - 7, topLeft.y + 287)));				
			drawLightArea(recti(topLeft + pos2di(size.width - 87, 288), pos2di(botRight.x - 7, topLeft.y + 378)));			
			drawHSep(recti(topLeft + pos2di(278, 257), dim2di(size.width - 284, 7)));
			drawHSep(recti(topLeft + pos2di(278, 377), dim2di(size.width - 284, 7)));	
			drawHSep(recti(topLeft + pos2di(size.width - 89, 283), dim2di(83, 7)));
			drawHSep(recti(topLeft + pos2di(278, 196), dim2di(size.width - 284, 7)));
			drawVSep(recti(topLeft + pos2di(335, 198), dim2di(7, 61)));		
			drawVSep(recti(topLeft + pos2di(size.width - 90, 259), dim2di(7, 120)));
			drawVSep(recti(topLeft + pos2di(517, 168), dim2di(7, 30)));
			
			recti impprogressArea(topLeft + pos2di(521, 168), pos2di(botRight.x - 8, topLeft.y + 168 + 29));

			int newWidth = floor((impprogressArea.getSize().width - 4) / 12) * 12 + 4;
			impprogressArea = recti(impprogressArea.UpperLeftCorner + pos2di((impprogressArea.getSize().width - newWidth) / 2, 0),
								dim2di(newWidth, 29));

			drawProgressBar(impprogressArea, max(impprogress, 0.f), impprogress >= 0 ? Color(0xffffffff) : Color(0xaaffffff));			
		}
	...
	void update(float time) {
		...
		float terCnt = 0.f, terMax = 0.f, temp = 0.f;
		obj.getStateVals(strTerraform, terCnt, terMax, temp, temp);
		structuresText.setText(combine(i_to_s(strCnt), "/", f_to_s(planet.getMaxStructureCount(), 0), " ", localize("#slots")) + ", " + combine(i_to_s(int(terMax - terCnt)), "/", i_to_s(int(terMax)), " ", localize("#terslots")));
		refreshCurStructs();
		...
		if (queueSize != lastQueueSize || queueSize > 0) {
			...
			if (queueSize > 0) {
				...
				obj.getConstructionCost(0, strFuel, done, req);
				activeCost.update(4, done, req);

				obj.getConstructionCost(0, strAmmo, done, req);
		...
		if(obj.getFlag(objImprovement)) {
			float iM = 0.f, iE = 0.f, iA = 0.f, tM = 0.f, tE = 0.f, tA = 0.f, tmp = 0.f;
			if(obj.getStateVals(strCosts, iM, iE, iA, tmp) && obj.getStateVals(strTotalCosts, tM, tE, tA, tmp)) {
				float total = tM + tE + tA, current = iM + iE + iA;
				if(current >= 0.01f) {
					impprogress = 1 - (current / total);
					improvBuild.setText(combine("Building : ", impName, " (", f_to_s(impprogress * 100.f, 0), "%)"));
				}	
				else
					impprogress = -1.f;
			}
			...
			impprogress = -1.f;
			improvBuild.setText(combine("Building : ", localize("#PL_NOTHING"), ""));			
		}
	...
	void switchTab(int num) {
		...
		improvTab.setPressed(num == 2);
		econTab.setPressed(num == 3);

		queuePanel.setVisible(num == 0);
		structsPanel.setVisible(num == 1);
		improvPanel.setVisible(num == 2);		
		econPanel.setVisible(num == 3);
	...
	EventReturn onGUIEvent(GuiElement@ ele, const GUIEvent& evt) {
		...
		switch (evt.EventType) {
			...
			case GEVT_Clicked:
				if (evt.Caller is close) {
				...
				else if (evt.Caller is improvTab) {
					switchTab(2);
					return ER_Pass;
				}
					...
					if (evt.Caller is terraTab) {
						terraTab.setPressed(true);
	
						terraList.setVisible(true);
						setGuiFocus(terraList);
						return ER_Pass;
					}
					else if (evt.Caller is improvPause) {
						pauseImprov();
						return ER_Pass;
					}
					else if (evt.Caller is improvCancel) {
						cancelImprov();
						return ER_Pass;
					}
				}
			...
			case GEVT_Listbox_Selected_Again:
				...
				else if (evt.Caller is terraList) {
					buildTImprovement();
					return ER_Pass;
				}
			break;
	...
	/* }}} */
	/* {{{ Improvements*/
	/* 0 = Uninitilized, 1 = Unlocked, 2 = Used, 3 = Unavailable */
	void updateTerrestial() {
		const Empire@ emp = getActiveEmpire();
		uint sysCount = emp.getSubSysDataCnt();
		uint terCount = 0;
		
		for(uint i = 0; i < sysCount; ++i) {
			const subSystemDef@ temp = emp.getSubSysData(i).type;	
			if(temp.hasTag("Terrestial")) {
				terCount++;
			}
		}
		
		if(lastTerrestial != terCount) {
			lastTerrestial = terCount;
			terraList.clear();
			
			for (uint i = 0; i < sysCount; ++i) {
				const subSystemDef@ def = emp.getSubSysData(i).type;
				if(def.hasTag("Terrestial")) {
					terraList.addItem(def.getName());
					if(!available.exists(def.getName()))
						available.set(def.getName(), 0);
				}
			}
		}
	}
	
	void updateAvailability(const subSystemDef@ def) {
		Object@ obj = planet;
		uint built;

		if(available.get(def.getName(), built)) {
			if(built == 0)
				available.set(def.getName(), 1);					
				
			// Check if there are free terraforming slots.
			float max = 0.f, val = 0.f, req = 0.f, carg = 0.f;
			if(def.hasTag("Former")) {
				obj.getStateVals(strTerraform, val, max, req, carg);	
				if(val == 0)
					available.set(def.getName(), 2);
				else if (val <= max)
					available.set(def.getName(), 1);
			} 						
		}
	...
	void refreshTerraInformation() {
		int itemSelected = terraList.getSelected();
		uint constructable;
		if(itemSelected >= 0) {
			const subSystemDef@ def = getSubSystemDefByName(terraList.getItem(itemSelected));

			updateAvailability(def);
			available.get(def.getName(), constructable);	
						
			string@ avail = "", desc = "", name = "", cost = "";
			if(constructable == 1) {
				avail += "\n#c:green#("+localize("#PL_UNLOCKED")+")#c#";
				unlockIcon.setVisible(true);
				lockIcon.setVisible(false);
			} else if (constructable == 2) {
				avail += "\n#c:yellow#("+localize("#PL_USED")+")#c#";
				lockIcon.setVisible(true);
				unlockIcon.setVisible(false);
			} else {
				avail += "\n#c:orange#("+localize("#PL_LOCKED")+")#c#";	
				lockIcon.setVisible(true);
				unlockIcon.setVisible(false);
			}
			name += "#font:frank_12#"+terraList.getItem(itemSelected)+"#font#\n";
			name += "#c:green#("+localize("#PL_TERRESTRIAL")+")#c#";
			name += avail;
			improvName.setText(name);
			
			desc += def.getDescription();
			improvDesc.setText(desc);
			...
			if(factory.generateSubSystems(def, null)) {
				...
				improvCost.update(0, subsys.getCost(2));
				improvCost.update(1, subsys.getCost(1));
				improvCost.update(2, subsys.getCost(0));	
				improvCost.setVisible(true);
			}
		}
	}
	
	void buildTImprovement() {
		const Empire@ emp = getActiveEmpire();
		uint sysCount = emp.getSubSysDataCnt();	
		int itemSelected = terraList.getSelected();
		uint constructable;
		...
		/* Do not allow improvements to be started if structures are being built */
		uint cnt = obj.getConstructionQueueSize();
		for (uint i = 0; i < cnt; ++i) {
			string@ type = obj.getConstructionType(i);
			if (@type != null && type == "structure")
		...
		/* If no structures are being built go on a start the improvement */
		if(itemSelected >= 0 && !obj.getFlag(objImprovement)) {
			const subSystemDef@ def = getSubSystemDefByName(terraList.getItem(itemSelected));

			available.get(def.getName(), constructable);

			if(constructable == 3 || constructable == 2 || constructable == 0)
				return;
			...
			if(factory.generateSubSystems(def, null)) {					
				...
				//Improves the Planet Quality.
				if(def.hasTag(strFormer)) {
					string@ sID = def.getName();
					@impName = def.getName();
					sendServerMessage("GAImprovements", 1, sID, obj.uid);
					refreshTerraInformation();
				}
			}	
		}							
	...
	void pauseImprov() {
		...
		if(obj.getFlag(setImpPause)) {
			obj.setFlag(setImpPause, false);
			improvPause.setPressed(false);
		}
		else {
			obj.setFlag(setImpPause, true);	
			improvPause.setPressed(true);
		}
	}
	...
	void cancelImprov() {
		...
		//Clear the pause flag on cancel
		if(obj.getFlag(setImpPause)) {
			obj.setFlag(setImpPause, false);
			improvPause.setPressed(false);
		}		
	
		sendServerMessage("GAImprovements", 0, "", obj.uid);	
	}
	...
	void updateEconText(bool updateText) {
		...
		float gotGuds = 0.f, gotLuxs = 0.f, h3fulGen = 0.f, h3fulTrans = 0.f;
		...
		obj.getStateVals(strH3FuelGen, h3fulGen, temp, temp, h3fulTrans);

		advAvg = movingAvg(advGen, advAvg);
		mtlAvg = movingAvg(mtlGen, mtlAvg);
		elcAvg = movingAvg(elcGen, elcAvg);
		fulAvg = movingAvg(fulGen+h3fulGen, fulAvg);
		...
		fulExpAvg = movingAvg(fulTrans+h3fulTrans, fulExpAvg);
		amoExpAvg = movingAvg(amoTrans, amoExpAvg);

		foodCons = float(population * (0.06 / million)) + (fulGen);
		mtlCons = (amoGen / 2) + advGen + (elcGen * 2.f);
		elcCons = advGen;
		...
		if (updateText) {
			...
			if (obj.getStateVals(strFuel, val, max, temp, cargo))
				updateStorage(6, val+cargo, max);
			if (obj.getStateVals(strAmmo, val, max, temp, cargo))
				updateStorage(7, val+cargo, max);				
			...
			updateRate(6, fulAvg+fulGen+h3fulGen, 0, -fulExpAvg);
		...
		if(itemHovered >= 0 && uint(itemHovered) < buildStructsList.getItemCount()) {
			...
			if(factory.generateSubSystems(def, null)) {
				...
				for(uint i = 0; i < hintCount; ++i) {
					...
					//Stop duplicate HP entry yet preserve the hint for the structures panel
					if(hintName == "HP")
						continue;
	...
	void buildShip() {
		...
		if(itemSelected >= 0) {
			...
			if(temp.hasSystemWithTag("ImportBay")) {
				const HullStats@ stats = temp.getStats();
	...
	void buildStructure() {
		if(planet.toObject().getFlag(objImprovement) && !planet.toObject().getFlag(setImpPause))
			return;
...
float gc_mtl = 0.f, gc_elc = 0.f, gc_adv = 0.f, gc_fud = 0.f, gc_ful = 0.f, gc_amo = 0.f;
...
void updateGC(){
	if (lastEmpire !is getActiveEmpire()) {
		...
		if(factory.generateSubSystems(def, null)) {
			...
			gc_ful = factory.active.getHint("FuelGen");
			gc_amo = factory.active.getHint("AmmoGen");
		}
	}
}
/* }}} */
