...
string[] branchNames = {"Theoretical Science", "Expansion", "Warfare", 
						"Ship Design", "Ship Systems", "Defensive Systems",
						"Ballistic Weapons", "Energy Weapons", "Self-Propelled Weapons"};
...
class ResearchWindow : ScriptedGuiHandler {
	...
	void init(GuiElement@ ele) {
		...
		@detailsText = GuiExtText(recti(pos2di(10,50), dim2di(231,313)), detailsBG);
		
		@branchList = GuiListBox(recti(7, 54, 0,0), true, ele);
		for(uint i = 0; i < branchNames.length(); i++)
		{
			branchList.addItem(branchNames[i]);
		}
		branchList.setSelected(0);
		...
		webPlane.setPosition(pos2di(planeWidth / -2 - itemMargin + (ele.getSize().width - 187 - 231),
		...
		for(int i = 0; i < webItemCount; ++i) {
			...
			if (desc.get_id().beginsWith("GenSci") 
				|| desc.get_id().beginsWith("Spatial")
				|| desc.get_id().beginsWith("Energy"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[0]));
			}	
			else if (desc.get_id().beginsWith("Col") 
				|| desc.get_id().beginsWith("Logistics") 
				|| desc.get_id().beginsWith("Infra"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[1]));
			}	
			else if (desc.get_id().beginsWith("Spy")
			    ||desc.get_id().beginsWith("Troop"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[2]));
			}	
			else if (desc.get_id().beginsWith("Maint")
			    || desc.get_id().beginsWith("Materials")
				|| desc.get_id().beginsWith("ShipCon"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[3]));
			}	
			else if (desc.get_id().beginsWith("Engi")
				|| desc.get_id().beginsWith("Propulsion")
				|| desc.get_id().beginsWith("WeapMod"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[4]));
			}			
			else if (desc.get_id().beginsWith("ActiveDef")
				|| desc.get_id().beginsWith("Shields")
				|| desc.get_id().beginsWith("Armor"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[5]));		
			}
			else if (desc.get_id().beginsWith("Ballistics")
				|| desc.get_id().beginsWith("Kinetic"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[6]));
			}			
			else if (desc.get_id().beginsWith("BeamEnergy")
				|| desc.get_id().beginsWith("ProjEnergy"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[7]));
			}
			else if (desc.get_id().beginsWith("Missiles")
				|| desc.get_id().beginsWith("Torpedo"))
			{
				regWebItem(WebGridItem(this, desc, int(pos.x), int(pos.y), branchNames[8]));
			}
		}
		...
		// Set items visible
		for(int i = 0; i < webItemCount; i++) {
			webItems[i].setVisible(branchNames[branchList.getSelected()]);
		}
	}
	...
	void matchGlowToImage(GuiImage@ img, bool glowVis) {
	...
	void syncPosition(dim2di size) {
		...
		rect = recti(pos2di(195, 54), pos2di(size.width - 231, size.height - 7));
		...
		branchList.setPosition(pos2di(6,55));
		branchList.setSize(dim2di(180, rect.getSize().height));
	...
	void update(ResearchWeb& web, float time) {
		// Update information in window
		{
			...
			if (activeTech is null) {
				...
				matchGlowToImage(activeItem.icon, activeItem.visible);	

				for(uint j = 0; j < branchList.getItemCount(); j++) {
					if(branchList.getItem(j).opEquals(activeItem.category))
						branchList.setItemOverrideColor(j, Color(0xff06dfff));
					else
						branchList.clearItemOverrideColor(j);
				}				
	...
	void draw(GuiElement@ ele) {
		...
		// Research region
		drawLightArea(recti(topLeft+pos2di(194,54), botRight-pos2di(231,7)));
		...
		drawVSep(recti(pos2di(topLeft.x+187,topLeft.y + 54), pos2di(topLeft.x + 194, botRight.y - 5)));
	...
	EventReturn onGUIEvent(GuiElement@ ele, const GUIEvent& evt) {
		...
		if (evt.Caller is branchList) {
			if(evt.EventType == GEVT_Listbox_Changed)
			{
				setBranch(branchList.getSelected());
			}
			else if(evt.EventType == GEVT_Focus_Gained)
			{
				branchList.bringToFront();
			}
		}		
	...
	void setBranch(int num) {
		for(uint i = 0; i < webItems.length(); i++) {
			webItems[i].setVisible(branchNames[num]);
		}
	}
};
/* }}} */
...
class WebGridItem : ScriptedGuiHandler {
	...
	WebGridItem(ResearchWindow@ Win, const WebItemDesc@ Descriptor, int hIndex, int vIndex, string@ Category) {
		@win = Win;
	...
	void setVisible(string Category) {
		visible = category.opEquals(Category);
	...
	void updateToolTip() {
		...
		if (@item != null) {
			win.detailsName.setText( "#font:frank_12##c:0d0#"+descriptor.name + "\n#c#("+localize("#RW_Level")+" " + level + ")#font#" );
			...
			win.detailsName.setText( "#font:frank_12##c:f00#"+descriptor.name + "\n#c#("+localize("#RW_Locked")+")#font#" );
