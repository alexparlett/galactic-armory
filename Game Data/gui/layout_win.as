...
bool makeDropPosition(pos2df& layPos, float scale, float extraDist) {
	float origDistance = sqrt(sqr(layPos.x) + sqr(layPos.y)) + sqrt(scale) / 2;
	bool validPosition = origDistance < maxRadius;
	float distance = origDistance - extraDist;

	if (origDistance > maxRadius && distance < maxRadius) {
		float factor = maxRadius / origDistance;
		layPos.x *= factor;
		layPos.y *= factor;
		return true;
	}
...
const string@ tagHull = "Hull", strHP = "HP", strMass = "Mass", tagStructure = "Structure", tagImprovement = "Improvement";
...
class LayoutWindow : ScriptedGuiHandler {
	...
	void init(GuiElement@ ele) {
		...
		// Quick stats
		@accelText = GuiStaticText(recti(325, 7, 405, 27), localize("#LE_Acceleration")+":", false, false, false, topPanel);
		@accelValue = GuiStaticText(recti(410, 7, 500, 27), "0.00", false, false, false, topPanel);

		@flightText = GuiStaticText(recti(325, 29, 405, 49), localize("#LE_FlightTime")+":", false, false, false, topPanel);
		@flightValue = GuiStaticText(recti(410, 29, 500, 49), "N/A", false, false, false, topPanel);

		// Cost icons
		@costTop = ResourceGrid(topPanel, pos2di(515, 4), dim2di(120, 17), 2);
		costTop.add(SR_AdvParts, 0);
		costTop.add(SR_Metals, 0);

		@costMiddle = ResourceGrid(topPanel, pos2di(515, 21), dim2di(120, 17), 2);
		costMiddle.add(SR_Electronics, 0);
		costMiddle.add(SR_Labor, 0);
		
		@costBottom = ResourceGrid(topPanel, pos2di(515, 37), dim2di(120, 17), 2);
	...
	void draw(GuiElement@ ele) {
		...
		// Draw top area
		drawWindowFrame(absPos);
		drawResizeHandle(recti(botRight - pos2di(19, 19), botRight));		drawTabBar(recti(topLeft + pos2di(6, TB_HEIGHT - 22), dim2di(size.width-12, 29)));
	...
	void clearLayout() {
		...
		costMiddle.update(0, 0);
		costMiddle.update(1, 0);		
	...
	void updateLayout() {
		...
		costMiddle.update(0, stats.getCost("Electronics"));
		costMiddle.update(1, stats.getCost("Labr"));		
		costBottom.update(0, stats.getCost("Fuel"));
		costBottom.update(1, stats.getCost("Ammo"));
		...
		if(stats.getHint("Control") < 0)
			setError(localize("#LEE_Control"), true, true);
		else if(stats.getHint("ArmorPoints") < 0)
			setError(localize("#LEE_Armor"), true, true);
		else if(stats.getHint("ShieldEmitters") < 0)
			setError(localize("#LEE_Shields"), true, true);
		else if(stats.getHint("ExternalMounts") < 0)
			setError(localize("#LEE_ExtMounts"), true, true);
		else if(stats.getHint("Air") < 0)
			setError(localize("#LEE_LifeSupport"), true, true);
		else if(stats.getHint("Crew") < 0)
			setError(localize("#LEE_Crew"), true, true);
		else if(stats.getHint("FuelUse") < 0 && stats.getHint("Fuel") <= 0)
			setError(localize("#LEE_Fuel"), true, false);
		else if(stats.getHint("Local/ImportTrade") > emp.getStat("importer"))
			setError(localize("#LEE_Trade"), true, false);					
		...
		else if(stats.getHint("FuelCosts") > stats.getHint("Fuel"))
			setError(localize("#LEE_Fuel"), false, false);			
		...
		else if(stats.getHint("AntimatterGeneration") < 0)
			setError(localize("#LEE_AntimatterGeneration"), false, false);
		else if(stats.getHint("WasteHeat") > 0)
			setError(localize("#LEE_WasteHeat"), false, false);	
	...
	void initLayoutView(GuiPanel@ ele) {
		...
		addList(localize("#LE_Power"), "Power", null, ele);
		addList(localize("#LE_Support"), "Support", null, ele);
		addList(localize("#LE_Storage"), "Storage", null, ele);
		addList(localize("#LE_Engines"), "Engine", null, ele);
		addList(localize("#LE_Defense"), "Defense", null, ele);
		addList(localize("#LE_Armor"), "Armor", null, ele);
		addList(localize("#LE_Shields"), "Shields", null, ele);
		addList(localize("#LE_ProjWeapons"), "ProjWeapon", null, ele);
		addList(localize("#LE_BeamAndEnergyWeapons"), "BeamEnergyWeapon", null, ele);
		addList(localize("#LE_MissileWeapons"), "MissileWeapon", null, ele);
		addList(localize("#LE_SpecWeapons"), "SpecWeapon", null, ele);
		addList(localize("#LE_FighterBomberWeapons"), "FighterWeapon", "BomberWeapon", ele);
		addList(localize("#LE_SubSystemModifiers"), "Link", null, ele);
		addList(localize("#LE_Docks"), "BankAccess", null, ele);
		...
		@sysCost = ResourceGrid(sysInfoPanel, pos2di(70, 44), dim2di(70, 17), 6);
		...
		if (layoutStats.hovered >= 0) {
			...
			if (layoutStats.hovered != prevHoveredStat) {
				...
				if (curNegStat > 0 || curPosStat > 0) {
					...
					if (entry.stat == strPointDefense)
						curPosStat = min(curPosStat, 0.8f);
				}
	...
	void initSettings(GuiPanel@ ele) {
		...
		GuiStaticText(recti(pos2di(6,30), dim2di(132, 19)), localize("#LE_MinHp"), false, false, false, parent).orphan(true);
		...
		GuiStaticText(recti(pos2di(212,30), dim2di(132, 19)), localize("#LE_MaxHp"), false, false, false, parent).orphan(true);
		...
		GuiStaticText(recti(pos2di(6,75), dim2di(132, 19)), localize("#LE_MinScale"), false, false, false, parent).orphan(true);
		@ai_trg_scale_min = GuiEditBox(recti(pos2di(6,95), dim2di(132, 19)), "0", true, parent );
		
		GuiStaticText(recti(pos2di(212,75), dim2di(132, 19)), localize("#LE_MaxScale"), false, false, false, parent).orphan(true);
	...
	GuiStaticText(recti(pos2di(6,146), dim2di(290, 19)), localize("#LE_defaultStance"), false, false, false, parent).orphan(true);
		...
		@ai_customDefendRange = GuiEditBox(recti(pos2di(482,171), dim2di(80, 19)), "40", true, parent );
		ai_customDefendRange.setVisible(false);

		@ai_customDefendCaption = GuiStaticText(recti(pos2di(567,172), dim2di(140, 19)), localize("#au"), false, false, false, parent);
	...
	void initStats(GuiPanel@ ele) {
		...
		statsPage.addItem(localize("#LD_AsteroidUsePage"));
		statsPage.addItem(localize("#LD_WasteHeatPage"));
		...
		@stat_btn6 = ToggleButton(false, recti(pos2di(930 - 735,11), dim2di(100, 18)), "", ele); stat_btn6.setID(stat_btn_id);
			...
			case GEVT_Clicked:
				if (evt.Caller.getID() == stat_btn1.getID()) {
					...
					if(evt.Caller is stat_btn1)
						...
						pageMode = 4;
					else if(evt.Caller is stat_btn6)
						pageMode = 5;
	...
	void RefreshDetailedStats(const HullLayout@ hull, HullStats@ hs) {
		...
		switch (page) {
			...
			break;
			case 5:
				BuildUsageStats(hull, hs, "AsteroidUse", "Asteroids");
			break;		
			case 6:
				BuildUsageStats(hull, hs, "WasteHeat", "Heatsink");
			break;				
			case 7:
	...
	void BuildDetailedStatsOverview(const HullLayout@ hull, HullStats@ hs) {
		...
		// Asteroids firing time
		float aster = hs.getHint("Asteroids");
		float asterUse = hs.getHint("AsteroidUse");

		if (aster > 0 && asterUse < 0) {
			text += "\n"+localize("#LD_FireTimeAsteroids");
			statsVals += "\n"+secondsToTime(aster/-asterUse);
		}
		...
		// Cumulative damage with asteroids
		if (aster > 0 && asterUse < 0) {
			float asterDPS = 0.f;
			uint cnt = hull.getSubSysCnt();
			for (uint i = 0; i < cnt; ++i) {
				const subSystem@ sys = hull.getSubSys(i);
				if (sys.hasHint("AsteroidUse"))
					asterDPS += sys.getHint("DPS");
			}

			float cumDPS = asterDPS * (aster / -asterUse);
			if (cumDPS > 0) {
				text += "\n"+localize("#LD_CumDPSAster");
				statsVals += "\n"+standardize(cumDPS);
			}
		}
		...
		//Heatsink stats
		float heatsink = hs.getHint("Heatsink");
		float wasteHeat = hs.getHint("WasteHeat");
		
		//Heatsink Capacity
		if(heatsink > 0){
			text += "\n\n\n"+localize("#LD_Heatsink");
			statsVals += "\n\n\n"+standardize(heatsink);
		}
		
		//Energyweapons consecutive firing time
		if (heatsink > 0 && wasteHeat > 0) {
			text += "\n"+localize("#LD_FireTimeEnergy");
			statsVals += "\n"+secondsToTime(heatsink/wasteHeat);
		}
	...
	void AddWeaponAsHeat(const subSystem@ sys, float& maximum) {
		float wasteHeat = sys.getHint("WasteHeat");
		
		if(wasteHeat <= 0)
			return;
		
		stat_graph.addStat(wasteHeat, sys.type.getImage(), combine(sys.type.getName(), " (", f_to_s(sys.scale, 2), ")"));
		maximum = max(wasteHeat * 1.2f, maximum);
	}
	...
	void AddWeaponAsAmmoUse(const subSystem@ sys, float& maximum) {
		...
		if(ammo <= 0){
			ammo = sys.getHint("AsteroidUse") * -1.f;
			stat_graph.setSuffix(localize("#LDS_AsteroidUse"));
			if(ammo <= 0)
				return;
		}
	...
	void BuildWeaponStats(const HullLayout@ hull, HullStats@ hs) {
		...
		for (uint i = 0; i < sysCnt; ++i) {
			...
			if(!def.hasTag("Weapon") && !def.hasTag("SuperWeapon"))
				continue;
			
			switch(mode) {
				case WS_Heat:
					stat_graph.setSuffix(localize("#LDS_Heat"));
					AddWeaponAsHeat(sys, maximum); 
					break;
				case WS_Range:
					stat_graph.setSuffix(localize("#LDS_AU"));
					//maximum = 0.25f;
					AddWeaponAsRange(sys, maximum); 
					break;
				case WS_DPS:
					stat_graph.setSuffix(localize("#LDS_DPS"));
					AddWeaponAsDPS(sys, maximum); 
					break;
				case WS_ShotDamage:
					stat_graph.setSuffix(localize("#LDS_Damage"));
					AddWeaponAsShotDamage(sys, maximum); 
					break;
				case WS_ShotDelay:
					stat_graph.setSuffix(localize("#LDS_Delay"));
					AddWeaponAsShotDelay(sys, maximum); 
					break;
				case WS_AmmoUse:
					stat_graph.setSuffix(localize("#LDS_AmmoUse"));
					AddWeaponAsAmmoUse(sys, maximum); 
					break;
			}
		}
		...
		prepStatButtons("#LDSB_Heat","#LDSB_Range","#LDSB_DPS","#LDSB_ShotDmg","#LDSB_ShotDelay","#LDSB_AmmoUse");
	...
	void prepStatButtons(const string@ text1, const string@ text2, const string@ text3, const string@ text4, const string@ text5, const string@ text6) {
		...
		stat_btn6.setVisible(text6 !is null);
		if(text6 !is null)
			stat_btn6.setText(localize(text6));
	}

	void hideStatButtons() {
		prepStatButtons(null,null,null,null,null,null);
	}
	/* }}} */
};
/* }}} */
